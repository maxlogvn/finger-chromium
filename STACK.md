...
**Lý do chọn:**
- Tương thích hoàn hảo với `playwright-core` -- dùng chung API `BrowserContext`, `Page`, `CDPSession`.
- `test.describe` / `test.afterAll` cho phép tổ chức suite test có lifecycle rõ ràng.
- `expect` API mạnh: `.toBe()`, `.toContain()`, `.toHaveLength()` cho kiểm tra kết quả browser.
- Hỗ trợ `--ui` mode cho phép debug test qua giao diện web.
